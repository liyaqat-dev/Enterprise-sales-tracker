...
/* STREAMING_CHUNK:Defining type definitions... */
interface Staff {
id: string;
name: string;
}
...
/* STREAMING_CHUNK:Initializing database client... */
...
/* STREAMING_CHUNK:Setting up component state... */
...
const [newTxStaff, setNewTxStaff] = useState('');
const [newTxDate, setNewTxDate] = useState(''); // NEW: State for manual date
...
/* STREAMING_CHUNK:Loading external dependencies... */
useEffect(() => {
// 1. Load Supabase
// @ts-ignore
if (typeof window !== 'undefined' && window.supabase) {
...
/* STREAMING_CHUNK:Configuring PWA events... */
useEffect(() => {
...
/* STREAMING_CHUNK:Fetching remote enterprise data... */
...
/* STREAMING_CHUNK:Defining utility handlers... */
...
/* STREAMING_CHUNK:Updating rates and logging transactions... */
...
/* STREAMING_CHUNK:Processing new transaction creation... */
...
// Use manually selected date or current time if empty
const selectedDate = newTxDate ? new Date(newTxDate) : new Date();

// Formatting for display string
const timestampStr = selectedDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + ' ' + selectedDate.toLocaleDateString();

// Formatting for ISO strictly for created_at insert to sync correctly with filters
...
const inserts = newTxItems.map(item => {
  ...
  return {
    ...
    created_at: isoDateString // Insert explicit date for robust filtering
  };
});
...
/* STREAMING_CHUNK:Executing filter and reporting logics... */
...
Syncing Enterprise Vault...
{(!isLibLoaded || !isXlsxLoaded) && (
Bootstrapping Database & Export Services...
)}
...
return (
    ...
    <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
        ...
        <button
          onClick={() => {
            setNewTxItems([]);
            // Pre-fill local current date for user convenience
            const nowLocal = new Date();
            nowLocal.setMinutes(nowLocal.getMinutes() - nowLocal.getTimezoneOffset());
            setNewTxDate(nowLocal.toISOString().slice(0, 16));
  ...
  {/* STREAMING_CHUNK:Rendering metrics and filters... */}
  <main className="max-w-7xl mx-auto px-6 py-8">
    ...
    {/* STREAMING_CHUNK:Rendering live ledger table... */}
  ...
  {/* STREAMING_CHUNK:Rendering footer and static modals... */}
  ...
  {/* STREAMING_CHUNK:Rendering disbursement form with date field... */}
  {isLogModalOpen && (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/95 backdrop-blur-md rounded-3xl w-full max-w-xl overflow-hidden border border-[#D5C9B7] shadow-2xl transition-all duration-300">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-[#5C4033] uppercase tracking-wide mb-1.5 flex justify-between">
                <span>2. Disbursement Date</span>
              </label>
              <input
                type="datetime-local"
                required
  ...
  {/* STREAMING_CHUNK:Rendering staff and config modals... */}
  {isConfigModalOpen && (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
